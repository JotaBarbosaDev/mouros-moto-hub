...
import { Database } from "@/integrations/supabase/types-new";
...
export interface TreasurerDashboard extends DashboardMetrics {}
  ...
  async reviewApproval(approvalId: string, request: ReviewApprovalRequest): Promise<void> {
    ...
    if (request.comment || request.suggested_values) {
      await this.addComment(approvalId, {
        ...
        comment_type: request.action === 'approve' ? 'approval_note' : 'revision_request',
        values: request.suggested_values || {},
