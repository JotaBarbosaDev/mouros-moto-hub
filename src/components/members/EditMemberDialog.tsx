...
export function EditMemberDialog({ member, open, onOpenChange, onSave }: EditMemberDialogProps) {
  ...
  // Fetch member's vehicles - usando useCallback para evitar recriações desnecessárias
  const fetchVehicles = useCallback(async () => {
    ...
    try {
      ...
      if (data) {
        ...
        const vehiclesData = (data as unknown as VehicleData[]).map(v => ({
          id: v.id,
          brand: v.brand,
          model: v.model,
          type: v.type as VehicleType,
          displacement: typeof v.displacement === 'string' ? parseInt(v.displacement, 10) : v.displacement as number,
      ...
      toast({
        title: "Erro",
        description: "Ocorreu um erro ao carregar os veículos.",
        variant: "destructive",
      });
    }
  }, [member, toast]);
  ...
  const handleAddVehicle = async (vehicle: Omit<Vehicle, 'id'>) => {
    ...
    try {
      ...
      // Se for, não envia para o Supabase, pois pode causar erro 400
      ...
      // Add to local state with proper type assertion
      const vehicleData = data as unknown as VehicleData;
      ...
      const newVehicle: Vehicle = {
        ...
        displacement: typeof vehicleData.displacement === 'string' 
          ...
          : vehicleData.displacement as number,
